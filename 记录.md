...
void pre(int root, int start, int end) {  // 已知后序和中序求前序的函数，root是对后序说的，start和end是对中序说的
	if (start > end) return;
	int i = start;
...
解题思路：像相加算术一样从后往前按位相加，处理一下进位就行

### 1059 Prime Factors

题目大意：给出一个整数，按照从小到大的顺序输出其分解为质因数的乘法算式，如97532468=2^2\*11\*17\*101\*1291

解题思路：  1、 用埃氏筛法建立素数表prime[]，范围 <=sqrt(n)即可；
2、建立表后，开始访问素数表，当可以整除时开始计数，并令n /= prime[i]，直至n % prime[i] != 0，不能被整除，此时即可输出；
3、当输出完 <= sqrt(n)内的，能整除n的素数时，若n ！= 1，此时输出该数即可。
...
1、一种我闻所未闻的找素数写法，全新写法，值得学习

2、将一个整数n分解为因子的函数

3、与素数相关的题，事先把所有素数计算出来并保存，是一件好事

### 1060 Are They Equal

题目大意：给出两个数，问将它们写成保留n位小数的科学计数法后是否相等。如果相等，输出YES，输出他们的科学记数法表示方法。如果不相等输出NO，分别输出他们的科学计数法

解题思路：转换成规定的科学计数法之后比较；话是这么说，但转换的过程中，需要全面仔细地考虑多种情况，如大量前导零，小数点等问题
...
1、测试数据可能不当人，此时要仔细全面地分析各种情况

2、将任意数字转换为n位有效数字的科学计数法表示，的函数

3、string似乎是用vector实现的（我猜的），因为string可以用push_back()函数

### 1061 Dating

题目大意：找一对字符串第一个相同的字符、第二个相同的字符，而且字符得在2个字符串下标相同时进行比较

解题思路：遍历这对字符串即可，记下第一个和第二个相同的字符，再去做一个映射，输出答案即可

**可以学习的：**

1、判断一个字符是不是数字或字母，用isdigit()函数和isalpha()函数

### 1062 Talent and Virtue

题目大意：分类排序；给出2个阈值L、H，第一类排序，virtue和talent均大于H，按照总分>virtue单项>编号的优先顺序，降序排序；第二类排序，virtue大于H，但talent下于H大于L，排序规则同第一类；第三类virtue和talent均小于H大于L，但vitrue需要大于talent，排序规则同第一类；第四类virtue和talent均小于H大于L，virtue小于talent，排序规则同第一类

解题思路：用结构体存储。写好3层的cmp函数；结构体数组vector v[4]中v[0]保存第一类考生，v[1]保存第二类考生…以此类推。写好cmp函数很重要，cmp函数中，排序先按照总分排序，然后按照德分排序，最后按照准考证号排序；最后输出符合条件的结果
...
1、比较函数cmp1()，在需要的时候可以有多层，目前见过最多的是3层

### 1063 Set Similarity

题目大意：给定两个整数集合，它们的相似度定义为：Nc/Nt*100%。其中Nc是两个集合的公共元素个数，Nt是两个集合的所有包含的元素个数。目的是计算任意一对给定集合的相似度

解题思路：直接用set存储；当需要计算集合a和集合b的相似度nc和nt的时候，遍历集合a中的每一个元素，寻找集合b中是否有该元素，这里的寻找用set的find()函数，复杂度是logn；如果有，说明是两个人公共的集合元素，则nc++，否则nt++（nt的初值为b集合里面本有的元素）
...
1、set的find()函数，其复杂度是logn；如果元素属于集合，则它返回确切的迭代器位置，否则返回.end()

2、printf输出百分号%，要在其前面再加一个%，即printf("%%")

### 1064 Complete Binary Search Tree

题目大意：给一串构成树的序列，已知该树是**完全二叉搜索树**，求它的层序遍历的序列

解题思路：关键就在这里的**完全二叉搜索树**，其具有优秀的性质：设根节点编号为root，起始为0，则左子树的根节点编号一定是2\*root+1，右子树的根节点编号一定是2*root+2。而这样的下标编号，正是层序遍历的顺序。因此，已知中序，从根节点开始中序遍历，按中序数组给出的顺序依次将值填入level数组对应的下标中，输出level数组可得层序遍历。
...
1、完全二叉搜索树的性质，其中序遍历的写法

### 1065 A+B and C (64bit)

题目大意：给3个范围在 (−$2^{63}$, $2^{63}$)的整数A，B，C，判断A+B是否大于C，是则输出true，否输出false

解题思路：~~用python写很方便~~ 这题也不需要真的去做大整数相加，因为只需要判断大小就行了，而2个long long相加的溢出是可预知和判断的。即：
...
1、2个整数相加溢出的情况下，溢出的结果的范围是多少；2个正整数相加溢出<= -2；2个负整数相加溢出>= 0；

### 1066 Root of AVL Tree

题目大意：AVL树是自平衡二叉搜索树。 在AVL树中，任何节点的两个子树的高度最多相差一；如果在任何时候它们相差多于一个，则重新平衡以恢复此属性。 图1-4说明了旋转规则；现在给出一系列插入，要求输出根节点的值

解题思路：写出建AVL（平衡二叉搜索树）的代码模版。涉及6个关键函数，其中4个处理不平衡时的旋转，包括leftLeftRotate、rightRightRotate、leftRightRotate、rightLeftRotate；1个获取AVL树的高度getHeight；1个较长的插入函数insert
...
题目大意：给出一个n个数的序列，数字为0~n-1的乱序，每次用两两交换的方式而且只能用0和另一个数交换，使序列变成有序的，问最少需要多少步

解题思路：模拟这个交换过程；用一个数组v记录每个元素的位置，序列有序当且仅当元素i位于第i位。一个直观的想法就是检查0所在的位置v[0]，这个位置应当属于v[0]，因此交换0和v[0]，它就入位了。需要考虑的是，0位于0位置有两种情况，一种是序列已经有序；另一种是虽然序列没有完全有序，但是0在0位置，这种情况，可以通过交换0和第一个不在正确位置的元素打破。那么，整道题的意思就是去找”打破“的次数。
...
解题思路：01背包问题；不过这里没有价值v，只有重量w，解决办法是直接把w[i]作为v[i]，因为这样的话如果求解过后dp\[n][V] == V的话，那么说明可以找到答案，否则就没有答案。怎么记录选择的硬币？可以同样开一个二维数组choice\[i][v]，chioce\[i][v] == 1说明付总价为v时选了i号硬币，否则就是没有选。最后再对choice\[i]\[m](i = 1,2,3…n)遍历找出其中为1的即可。答案不唯一的时候怎么找出字典序小的？比较复杂；从choice\[1][V]开始，可能会出现无解的情况；而反着来，从choice\[n][V]开始，可以避开无解的情况，因为这一项肯定是有解的
...
1、01背包问题的求解，有用二维数组的写法，也有用一维数组的写法，思路是一致的

2、01背包问题的路径输出，有时会有字典序的要求，那就反着排序，倒过来输出

### 1069 The Black Hole of Numbers

题目大意：给一个4位的数字，用其降序排列而成的数字减去其升序排列的数字，重复这个过程，直到结果为6174

解题思路：去模拟这个过程，补零用**s.insert(0, 4 – s.length(), ‘0’)**
...
1、string转int的函数，stoi()

2、int转string的函数，to_string()

3、字符串前面补零，s.insert(0, 4 - s.length(), '0'); 意思是在第0个位置前面插入4 - s.length()个的‘0’

### 1070 Mooncake

题目大意：给出n种月饼的数量和总价，并告知市场最大需求量d，求最大利润

解题思路：先算单价，从大到小排序，从最大的开始卖，累加即可得结果

### 1071 Speech Patterns

题目大意：统计单词个数；大小写字母+数字的组合才是合法的单词，给出一个字符串，求出现的合法的单词的个数最多的那个单词，以及它出现的次数。如果有并列的，那么输出字典序里面的第一个

解题思路：用map来做；isalnum()函数判断一个字符是不是字母或数字（自己写if也行，麻烦）
...
1、用C++分割长字符串，跟python的s.split(' ')函数差不多

2、判断是不是字母用isalpha()，判断是不是数字用isdigit()，判断是不是字母或数字用isalnum()，

### 1072 Gas Station

题目大意：从m个加油站里面选取1个站点，让他离居民区的最近的人最远，并且没有超出服务范围ds之内。如果有很多个最远的加油站，输出距离所有n个居民区距离平均值最小的那个。如果平均值还是一样，就输出按照顺序排列加油站编号最小的那个

解题思路：对n+m个点进行Dijkstra计算最短路径；
要求计算出1~m号加油站距离其他站点的最短路径。这时候可以遍历dis数组，如果dis存在一个距离大于服务范围ds的距离，那么我们就舍弃这个加油站。取最最短的路径，这就是距离它最近的加油站mindis。如果mindis > ansdis，就是说找到了一个距离居民最小距离的加油站是更远的，那就选这个加油站，更新ansid为它的id。最后输出。

晦气的是，题目没说2个点之间只有1条路径，也没说一定没有自环，所以在输入时，增加判断条件，只存储2个点之间最短的那条边，并且去除自环
...
题目大意：给出n个节点，每个节点有地址和next地址；现给出一个数k，目的是将每k个节点倒过来，不到k个的不用倒

解题思路：1.对输入对address，data，next进行存储
下面用到是结构体数组，node[address]，下标为address，结构体变量data,next
2.将反转链表简化为反转数组
再创建一个list数组对address按照顺序存储，对K个数内部进行反转
3.根据反转后对数组，找到关联对data,next进行输出
...
1、反转一个常为k的数组，只需进行 k / 2 次的交换即可

2、链表题，用结构体存储节点；先遍历一遍，将连起来的链表节点的地址依次存在数组中；后面基本上对这个数组进行操作就行了，因为节点是一个整体，操作其地址即可

### 1076 Forwards on Weibo

题目大意：给出每个用户关注的人的id，和转发最多的层数；目的是求出，当一个id发了条微博，最多会有多少个人转发

解题思路：带层数的广度优先，因为一个用户只能转发一次，所以用inq判断当前结点是否入队过了，如果入队过了就不能重复入队（重复转发消息），queue的数据类型必须为node，同时保存它的id和layer层数，控制不超过L层
...
1、广度优先遍历的写法，bfs，核心是用队列实现，不需要递归，一个while再套一个for就能完成

### 1077 Kuchiguse

题目大意：给定n个字符串，求他们的公共后缀，如果不存在公共后缀，就输出“nai”

解题思路1：先全部翻转，再用2个for遍历求最长前缀；虽然暴力，但由于这题字符串较短，也可以过

解题思路2：输入时将后缀翻转；以第一个字符串为基准，后面每来一个字符串都去跟第一个比较，然后截取相同的那部分
...
1、输入相关的一个问题；当需要第一行读入一个整数n，第二行读入一个带空格的字符串时，如果先scanf("%d", &n); 再getline(cin, s); 这样是达不到预期的，因为getline会把第一行的回车也读入到s中；正确的写法是scanf("%d\n", &n); 即要把第一行的回车给“吃掉”，再去getline

### 1078 Hashing

题目大意：给出散列表长和要插入的元素，将这些元素按照读入的顺序插入散列表中，其中散列函数为h(key) = key % TSize，解决冲突采用只向正向增加的二次方探查法。如果题中给出的TSize不是素数，就取第一个比TSize大的素数作为TSize

解题思路：模拟二次探查法解决哈希冲突的过程即可
...
1、Hash散列的概念，解决冲突的方法：线性探测和二次探测；最容易理解的是线性探测，冲突了往后移即可，而二次探测就是在线性探测的基础上，将往后移i位，变成往后移 $i^2$ 位

### 1079 Total Sales of Supply Chain

题目大意：给一棵树，在树根处货物的价格为p，然后从根结点开始每往下走一层，该层的货物价格将会在父亲结点的价格上增加r%，给出每个叶结点的货物量，求他们的价格之和

解题思路：树的遍历，可以采用dfs或者bfs两种方法。采用dfs，建立结构体数组保存每一个结点的孩子结点的下标，如果没有孩子结点，就保存这个叶子结点的data（销售的量）。深度优先遍历的递归出口，即当前下标的结点没有孩子结点的时候，就把ans += data（货物量）* pow(1 + r, depth)计算货物量*价格引起的涨幅百分比。如果有孩子结点，就dfs深度优先遍历每一个孩子结点，并且在当前depth层数的基础上+1。最后输出ans * p（销售价格），即总价格
...
1、树的储存方法有很多，这题见到了全新的存储方法：用结构体数组存储节点的值和其儿子节点的编号

2、dfs遍历树，对树的存储结构要求很宽松，有值、有儿子就行

### 1081 Rational Sum

题目大意：给出n个分数，求他们的和，分成整数部分和真分数部分分别输出

解题思路：先根据分数加法的公式累加，后分离出整数部分和分数部分；分子和分母都在长整型内，所以不能用int存储，否则有一个测试点不通过，得用long long；为了防止溢出，累加每一步后都进行分子分母的约分处理
